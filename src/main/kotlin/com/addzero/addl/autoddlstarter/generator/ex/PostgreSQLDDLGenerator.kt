...
class PostgreSQLDDLGenerator : DatabaseDDLGenerator() {
    private val fieldMappings: List<FieldMapping> = Companion.fieldMappings // 你需要初始化并设置字段类型映射关系
    ...
    override fun generateCreateTableDDL(ddlContext: DDLContext): String {
        ...
        println(
            "-----------开始生成建表语句,依据:$simpleName---------------当前读取的对象---$sheetName-----------------------"
        )
        val createTable = "create table if not exists $tableEnglishName ($lineSeparator"
        ...
        dto.stream()
            ...
            .forEach(Consumer<RangeContext> { rangeContext: RangeContext ->
                val fieldName: String = rangeContext.fieldName
